...
exports.structure = async (req, res) => {
  auth(req, res, () => {
    ...
    DataMiddleware.getTables((tables) => {
      data.tables = tables;
      DataMiddleware.getStructure(data.subtitle, (structure) => {
...
exports.removeColumn = async (req, res) => {
  auth(req, res, async () => {
    try {
      ...
      const existingTable = await db.sequelize.getQueryInterface().showAllTables();
      ...
      const tableColumns = await db.sequelize.getQueryInterface().describeTable(table_name);
      console.log(tableColumns);
      console.log(tableColumns[field_name]);
      if (!tableColumns[field_name]) {
        return res.status(500).json({ error: 'Column does not exist in the table' });
      }

      // Supprimer la colonne
      await db.sequelize.getQueryInterface().removeColumn(table_name, field_name);
...
exports.structure_edit = async (req, res) => {
  auth(req, res, async () => {
    try {
      const { table_name, structure_name, structure_type, structure_isnull } = req.body;
      ...
      if (tableColumns[table_name]) {
        return res.status(500).json({ error: 'Column already exists in the table' });
      }

      // Ajouter la colonne en fonction du type
      switch (structure_type) {
        case 'number':
          await db.sequelize.getQueryInterface().addColumn(table_name, structure_name, {
  ...
  auth(req, res, async () => {
    ...
    DataMiddleware.getTables((tables) => {
      ...
      DataMiddleware.getStructure(data.subtitle, (structure) => {
        ...
        DataMiddleware.getData(data.subtitle, (tableData) => {
    ...
    DataMiddleware.getTables((tables) => {
      ...
      DataMiddleware.getStructure(data.subtitle, (structure) => {
        ...
        for (let s in data.structure) {
          data.structure[s].template = data.structure[s].type.toLowerCase().replace(/\(.*\)/, '');
        }
        if (data.id) {
          DataMiddleware.getDataById(data.id, data.subtitle, (formData) => {
